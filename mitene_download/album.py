...
async def process_album(
    ...
    resolved_cache: DatabaseCache = db_cache if db_cache is not None else DatabaseCache(options.db_path)
    await resolved_cache.init_db()
        ...
        async def consume() -> None:
            while True:
                ...
                try:
                    if item is None:
                        return
                    await _handle_item(session, client, resolved_cache, item, options, summary)
                finally:
                    queue.task_done()
...
async def _handle_item(
    ...
    if result.outcome is Outcome.DOWNLOADED and result.path is not None:
        ...
        apply_metadata(final_path, item, write_exif_data=options.write_exif)
        if options.sidecar:
            write_sidecar(final_path, item)
